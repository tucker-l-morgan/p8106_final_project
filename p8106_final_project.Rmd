...
```{r combined eda figure, eval = TRUE, include = T}
...
```{r correlation plot, eval = TRUE, include = T, fig.width=10, fig.height=7}
missing_data_viz + (model.matrix(~0 + ., data = cleaned_df) %>%
  cor(use = "pairwise.complete.obs") %>% 
  ggcorrplot(show.diag = F, type = "lower", lab = TRUE, lab_size = 1, title = "Fig.2: Correlation of Predictors"))
```
...
```{r resampling train results, include = TRUE, fig.height=10, fig.width=10}
# Results from resampling on training data 
...
```{r training result table, include = TRUE}
table_result <-
