...
```{r, include=T, fig.width=12, fig.height=14}
...
vip(nnet_fit_impute$finalModel)
...
#4 <- varImp(GAM_fit_impute$finalModel)

#gplot(V4, aes(x = reorder(rownames(V4),Overall), y = Overall)) +
# geom_point(color = "blue", size = 2, alpha = 0.6) +
...
ROC_df <-  
  ...
  pivot_longer(Random_Forest:MARS, names_to = "method", values_to = "values")
...
plot(roc_rf_recipes_impute, col = P36[14])
plot(roc_boost_impute, add = TRUE, col = P36[15])
plot(roc_glmnet, add = TRUE, col = P36[16])
plot(roc_cart_impute, add = TRUE, col = P36[17])
plot(roc_cit_impute, add = TRUE, col = P36[18])
plot(roc_svm_linear_impute, add = TRUE, col = P36[19])
plot(roc_svm_radial_impute, add = TRUE, col = P36[20])
plot(roc_lda, add = TRUE, col = P36[21])
plot(roc_nnet_impute, add = TRUE, col = P36[22])
plot(roc_mars_impute, add = TRUE, col = P36[23])
#plot(roc_gam_impute, add = TRUE, col = 13)
...
auc <- sort(c(roc_rf_recipes_impute$auc[1], roc_boost_impute$auc[1], 
              ...
              roc_lda$auc[1], roc_nnet_impute$auc[1], roc_mars_impute$auc[1]), 
            decreasing = TRUE)
#roc.gam.impute$auc[1]

model_names = c("MARS", "Elastic Net", "Neural Network", "LDA", 
