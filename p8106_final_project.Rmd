...
vip_grid <- plot_grid(g2, g5, g3, g6, ncol = 2)
title <- ggdraw() + 
  draw_label(
    "Fig.4: Important Variables for Various Models",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
...
result2 <-
  ...
  stat_summary(fun.y = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..),
               width = 0.75, size = 1, linetype = "solid") + 
  coord_flip() +
  labs(title = "Distribution of the CV Estimated Sensitivity", 
...
result3 <-
  ...
  stat_summary(fun.y = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..),
               width = 0.75, size = 1, linetype = "solid") + 
  coord_flip() +
  labs(title = "Distribution of the CV Estimated Specificity", 
...
result1/(result2 +result3) + 
  plot_annotation(title = "Fig.5: Cross-Validation Results")
```

```{r training result table, include = FALSE, echo = FALSE}
table_result <-
...
plot(roc_glmnet, col = P36[1], print.auc = TRUE, legacy.axes = TRUE)
title(main = "Fig.6: ROC Curves for Prediction of Test Data", line = 3)
# plot(roc_rf_recipes_impute, add = TRUE, col = P36[32])
# plot(roc_boost_impute, add = TRUE, col = P36[32])
# plot(roc_cart_impute, add = TRUE, col = P36[32])
# plot(roc_cit_impute, add = TRUE, col = P36[32])
# plot(roc_svm_linear_impute, add = TRUE, col = P36[32])
# plot(roc_svm_radial_impute, add = TRUE, col = P36[32])
# plot(roc_lda, add = TRUE, col = P36[32])
# plot(roc_nnet_impute, add = TRUE, col = P36[32])
# plot(roc_mars_impute, add = TRUE, col = P36[32])
# plot(roc_gam_impute, add = TRUE, col = P36[32])
...
# legend("bottomright", legend = paste0("Elastic Net", ": ", round(roc_glmnet$auc[1],4)),
#        col = P36[1], lwd = 2)
```

In **Figure 6**, we see the area-under-curve of our selected model applied to our test data with a value of 0.7403, indicating middling classification performance. Despite our efforts with data imputation, it looks like our model still suffers due to class imbalance. The selected model coefficients can be seen in **Table 1**, and changes over the penalty tuning parameter can be seen in **Figure 7**. The predicted outcome for this model is CHD negative, so we can see that: female subjects are less likely to be CHD present than male subjects; as the number of cigarettes per day increases we expect the probability of CHD present to increase; the prevalence of hypertension contributes to increased odds of CHD in 10 years of follow up; and an increase in systolic blood pressure is expected to increase the probability of CHD present.

```{r glmnet coef table, echo = FALSE, include = TRUE}
coef(logit_next$finalModel, logit_next$finalModel$lambdaOpt) %>% 
  as.matrix() %>% 
  data.frame() %>% 
  filter(s1 != 0) %>% 
...
plot_glmnet(logit_next$finalModel, label = T,
            s = 0.016, main = "Fig.7: Trace Plot of Elastic Net Coefficients")
```

The table and the plot above show the feature selection result of our final model. Among selected predictors, some of their coefficients are close to zero and it appears that systolic blood pressure, age, average number of cigarettes per day, hypertension prevalence, and sex are the important factors in predicting whether one would have chronic heart disease or not. These predictors are also frequently high-ranked in variable importance results of the other models.
...
### Neural Network Model Plot with two Hidden Units

```{r, include=T, echo = FALSE, fig.width=20, fig.height=10}
plotnet(nnet_fit_impute$finalModel, main = "Neural Network Model Plot")
```

### MARS Model Partial Dependence Plot with Two Most Frequently Top-Ranked in VIP

```{r, include=T, echo=FALSE, fig.height=4, fig.width=6}
pdp::partial(mars_fit_impute, pred.var = c( "sys_bp", "age"),
                   grid.resolution = 10) %>%
      pdp::plotPartial(levelplot = FALSE, zlab = "yhat", drape = TRUE,
                       screen = list(z = 20, x = -60), main)
```

### All Model Application to Test Data
...
plot(roc_rf_recipes_impute, col = P36[22])
plot(roc_boost_impute, add = TRUE, col = P36[21])
plot(roc_glmnet, add = TRUE, col = P36[17])
plot(roc_cart_impute, add = TRUE, col = P36[24])
plot(roc_cit_impute, add = TRUE, col = P36[23])
plot(roc_lda, add = TRUE, col = P36[20])
plot(roc_nnet_impute, add = TRUE, col = P36[18])
plot(roc_mars_impute, add = TRUE, col = P36[16])
plot(roc_gam_impute, add = TRUE, col = 19)

auc <- sort(c(roc_rf_recipes_impute$auc[1], roc_boost_impute$auc[1], 
              roc_glmnet$auc[1], roc_cart_impute$auc[1],
              roc_cit_impute$auc[1],roc_lda$auc[1], 
              roc_nnet_impute$auc[1], roc_mars_impute$auc[1],roc_gam_impute$auc[1]),
            decreasing = TRUE)
