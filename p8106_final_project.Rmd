...
In **Figure 6**, we see the area-under-curve of our selected model applied to our test data with a value of 0.7403, indicating middling classification performance. Despite our efforts with data imputation, it looks like our model still suffers due to class imbalance. The selected model coefficients can be seen in **Table 2**, and changes over the penalty tuning parameter can be seen in **Figure 7**. The predicted outcome for this model is CHD negative, so we can see that: female subjects are less likely to be CHD present than male subjects; as the number of cigarettes per day increases we expect the probability of CHD present to increase; the prevalence of hypertension contributes to increased odds of CHD in 10 years of follow up; and an increase in systolic blood pressure is expected to increase the probability of CHD present.
