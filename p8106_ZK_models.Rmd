...
preprocess_recipe = recipe(ten_year_chd ~ ., data = training_df) %>%
  ...
  step_BoxCox(all_numeric_predictors(), -cigs_per_day) %>% # transform predictors; no Box-Cox transformation could be estimated for cigs_per_day, resulting in error, so omitting
  step_center(all_numeric_predictors()) %>% # center and scale numeric predictors
  step_scale(all_numeric_predictors())

prep = prep(preprocess_recipe, training = training_df)
training_pp = bake(prep, new_data = training_df)

training_pp %>% vis_miss()
training_pp
```
...
plot(roc_rf_impute, col = 1)
plot(roc_boost_impute, add = TRUE, col = 2)
plot(roc_glmnet, add = TRUE, col = 3)

auc = c(roc_rf_impute$auc[1], roc_boost_impute$auc[1], roc_glmnet$auc[1])

model_names = c("Random Forest w/ Impute", "Adaboost w/ Impute", "GLMNet w/ Impute")
